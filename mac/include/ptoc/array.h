...
class conf_array { 
  public:
    ...
    conf_array(int l, int h, Type* ptr) 
    ...
    int size_in_bytes() const { return (high-low+1)*sizeof(Type); }
...
class array {
  public:    
    ...
    int size() const { return high_bound - low_bound + 1; }
...
class conf_matrix {
  public:     
    const int low1, high1, low2, high2;

    conf_matrix(int l1, int h1, int l2, int h2, Type* ptr)
    ...
    conf_array<Type> operator[](int index) {
...
template<int low_1,int high_1,int low_2,int high_2,class Type>
class matrix { 
  public:
    array<low_2,high_2,Type> buf[high_1-low_1+1];

    array<low_2,high_2,Type>& operator[](int index) {
	assert(BETWEEN(low_1, index, high_1)); 
	return buf[index-low_1]; 
    }

    array<low_2,high_2,Type> const& operator[](int index) const {
	assert(BETWEEN(low_1, index, high_1)); 
	return buf[index-low_1]; 
    }
    ...
    int size()  const { return (high_1-low_1+1)*(high_2-low_2+1); }
...
template<int lo1,int hi1,class Type>
class array1d {
public:
    Type buf[hi1-lo1+1];
	
    Type& operator[](int index) {
		assert(BETWEEN(lo1, index, hi1));
		return buf[ index - lo1 ];
    }
    Type const& operator[](int index) const {
		assert(BETWEEN(lo1, index, hi1));
		return buf[ index - lo1 ];
    }
    int size()  const { return hi1-lo1+1; }
    Type* body() { return (Type*)buf; }
    Type const* body() const { return (Type const*)buf; }
};

template<int lo1,int hi1,int lo2,int hi2,class Type>
class array2d {
public:
    array1d<lo2,hi2,Type> buf[hi1-lo1+1];
	
    array1d<lo2,hi2,Type>& operator[](int index) {
		assert(BETWEEN(lo1, index, hi1));
		return buf[index-lo1];
    }
    array1d<lo2,hi2,Type> const& operator[](int index) const {
		assert(BETWEEN(lo1, index, hi1));
		return buf[index-lo1];
    }
    int size()  const { return (hi1-lo1+1)*(hi2-lo2+1); }
    Type* body() { return (Type*)buf; }
    Type const* body() const { return (Type const*)buf; }	
};

template<int lo1,int hi1,int lo2,int hi2,int lo3,int hi3,class Type>
class array3d {
public:
    array2d<lo2,hi2,lo3,hi3,Type> buf[hi1-lo1+1];
	
    array2d<lo2,hi2,lo3,hi3,Type>& operator[](int index) {
		assert(BETWEEN(lo1, index, hi1));
		return buf[index-lo1];
    }
    array2d<lo2,hi2,lo3,hi3,Type> const& operator[](int index) const {
		assert(BETWEEN(lo1, index, hi1));
		return buf[index-lo1];
    }
    int size()  const { return (hi1-lo1+1)*(hi2-lo2+1)*(hi3-lo3+1); }
    Type* body() { return (Type*)buf; }
    Type const* body() const { return (Type const*)buf; }
};

template<int lo1,int hi1,int lo2,int hi2,int lo3,int hi3,int lo4,int hi4,class Type>
class array4d {
public:
    array3d<lo2,hi2,lo3,hi3,lo4,hi4,Type> buf[hi1-lo1+1];
	
    array3d<lo2,hi2,lo3,hi3,lo4,hi4,Type>& operator[](int index) {
		assert(BETWEEN(lo1, index, hi1));
		return buf[index-lo1];
    }
    array3d<lo2,hi2,lo3,hi3,lo4,hi4,Type> const& operator[](int index) const {
		assert(BETWEEN(lo1, index, hi1));
		return buf[index-lo1];
    }
    int size()  const { return (hi1-lo1+1)*(hi2-lo2+1)*(hi3-lo3+1)*(hi4-lo4+1); }
    Type* body() { return (Type*)buf; }
    Type const* body() const { return (Type const*)buf; }
};
