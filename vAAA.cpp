...
int main (int argc, char * const argv[])
{
	...
	vector<string>	sv_inp;						// structual variation data input
...
//		else if (spl_in.empty()) spl_in=program.arg()[argi];
	...
	if (permte)
	{
		...
		if (vcf_in.empty()) exit_error("With --permute, input must be a file instead of the standard input.");
		wrWhat=perch::h_Pcrt;
	}
	else
	{
...
read_VCF:
	...
	for (Rows_in_File(in, vcf_in, &format))
	{
		...
		{
			if (AddInf) program.outf<<"##INFO=<ID="<<wrWhat<<",Number=1,Type=Float,Description=\"vAAA association analysis result.\">"<<endl;
		...
		if (trios.empty()) // old version, will never happen
		{
			...
			GTP::rewind(); for (size_t i=0;i<FldCSs.size();++i) { char g=GTP::read(in[FldCSs[i]],chr_num,bp,SexCSs[i],gpar); g_cs+=g; GTP::count(g,iiA,miA,niA,xiA,origWt[i]); }
			GTP::rewind(); for (size_t i=0;i<FldCTs.size();++i) { char g=GTP::read(in[FldCTs[i]],chr_num,bp,SexCTs[i],gpar); g_ct+=g; GTP::count(g,iiU,miU,niU,xiU,origWt[i+j]); }
		}
		else
		{
			size_t j=FldCSs.size();
			GTP::rewind(); for (size_t i=0;i<FldCSs.size();++i) { char g=_read_trio(tr_cs[i],in.contents(),chr_num,bp,DNonly,ndn,gpar); g_cs+=g; GTP::count(g,iiA,miA,niA,xiA,origWt[i]); }
			GTP::rewind(); for (size_t i=0;i<FldCTs.size();++i) { char g=_read_trio(tr_ct[i],in.contents(),chr_num,bp,DNonly,ndn,gpar); g_ct+=g; GTP::count(g,iiU,miU,niU,xiU,origWt[i+j]); }
		...
		char rsk='U'; // U=unknown A=alternative R=reference
		{
		...
		else if (showGT)
		{
			...
			string this_qcl;
			size_t j=FldCSs.size();
			for (size_t i=0;i<FldCSs.size();++i) if (GTP::usable(g_cs[i]) && GTP::num_alt(g_cs[i])) { this_qcl+=(this_var+DLMTR+record[i]  +"="+in[FldCSs[i]]+"\n"); }
			for (size_t i=0;i<FldCTs.size();++i) if (GTP::usable(g_ct[i]) && GTP::num_alt(g_ct[i])) { this_qcl+=(this_var+DLMTR+record[i+j]+"="+in[FldCTs[i]]+"\n"); }
		...
		else if (showIF)
		{
			...
			if (is_vks)
			{
				string this_var=in[0]+DLMTR+in[1]+DLMTR+in[2]+DLMTR+in[3]+DLMTR+in[4]+DLMTR+in[5]+DLMTR+in[6]+DLMTR+in[7]+DLMTR+GeneSymbol+DLMTR+in[FldFun[0]]+DLMTR+in[FldDet[0]];
				size_t j=FldCSs.size();
				for (size_t i=0;i<FldCSs.size();++i) if (GTP::usable(g_cs[i])) AAA_IF::to_rpt(GeneSymbol,GTP::num_alt_recessive(g_cs[i]),i,  (this_var+DLMTR+record[i]  +"="+in[FldCSs[i]]),this_qcl);
				for (size_t i=0;i<FldCTs.size();++i) if (GTP::usable(g_ct[i])) AAA_IF::to_rpt(GeneSymbol,GTP::num_alt_recessive(g_ct[i]),i+j,(this_var+DLMTR+record[i+j]+"="+in[FldCTs[i]]),this_qcl);
