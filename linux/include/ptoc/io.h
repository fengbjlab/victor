...
int pio_iostatus(file_descriptor *fd);
...
#ifdef __VMS
int pio_get_file_version(file_descriptor* fd);
#endif
...
void pio_input_int(text_descriptor* td, int* val); 
...
void pio_output_int(text_descriptor* td, int val, 
			const int* width); 
...
class file { 
  public:
    ...
    friend void reset(file& f, const char* file_name, 
		      const char* options, int& error_code) 
    { 
    ...
    friend void seek(file& f, int position) 
    {
...
class text : public file<char> { 
  public:
    ...
    text& operator >> (int& x) 
    {
	pio_input_int((text_descriptor*)this, &x);
	return *this;
    } 
    ...
    {
	int tmp;
	pio_input_int((text_descriptor*)this, &tmp);
	x = (short)tmp;
	return *this;
    } 
    ...
    text& operator << (int x) 
    {
	pio_output_int((text_descriptor*)this, x, NULL);
	return *this;
    } 
...
class format_int {
  ...
  public:
    friend text& operator << (text &t, format_int const& fi) { 
	pio_output_int((text_descriptor*)&t, fi.val, &fi.width);
	return t;
    }
    friend format_int format(int val, int width);
    format_int(int int_val, int int_width) { 
	val = int_val;
	width = int_width;
    }
};

inline format_int format(int val, int width) {
	return format_int(val, width);
}
